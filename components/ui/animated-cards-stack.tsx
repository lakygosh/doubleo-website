...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      ...
      /** LOCAL ADDITION — no chrome of its own; the caller styles the face
          with site CSS. Everything Double O drops in here is a `.tstack__card`. */
      bare: "flex size-full flex-col",
    },
  },
  defaultVariants: {
    variant: "light",
...
/**
 * The scroll track. Give it a tall height (e.g. `h-[300vh]`) and put a
 * `sticky top-0` child inside it: the child stays put while the page scrolls
 * through the track, and that scroll progress is what deals the cards.
 */
...
export const CardTransformed = React.forwardRef<HTMLDivElement, CardStickyProps>(
  (
    ...
    /* LOCAL PATCH: upstream calls useMotionTemplate inside a ternary on
       `variant`, so the hook is skipped entirely for the dark variant. That is
       a conditional hook — React throws the moment a card switches variant,
       and it breaks the rules-of-hooks lint besides. The template is always
       built now; only the value handed to the DOM is chosen. */
